module ctp_grid
  ...
  contains
    ...
    subroutine ctpgrid_init(this, mub0, lambda0, dlambda, npphin, neen, np)
        ...
        real :: ds, ctp_temp, ctp_lost_temp, on_axis_temp, min_ctp_E, max_ctp_E, dee, pphi_pos, offset
        ...
        i2 = 0
        ! Min/max energies to consider, depend on pitch angle
        ...
        ctp_temp = ctplost(mub0, getpphi(-1.)) ! Start at minimum of ctplost parabola
  
        do while (ctp_temp .lt. max_ctp_E) ! Keep searching until ctplost intersects maxE
            i1 = i1 + 1
            ctp_temp = ctplost(mub0, getpphi(-1 - ds * real(i1)))
        end do
  
        ! Find max pphi (right value) - where on axis E intersects min E
       ctp_temp = eeonaxis(mub0, getpphi(-1.)) 
       do while(ctp_temp .gt. min_ctp_E)
          i2 = i2 + 1 
          ctp_temp = eeonaxis(mub0, getpphi(-1 + ds * real(i2)))
      end do
  
      ! pphi range, don't include final index, it is the first "out of bounds" 
      this%i_pphi_start = i1 - 1! Left pphi index = # ds steps from -1 
      this%i_pphi_end = i2 - 1 ! Right pphi index = # ds steps from -1
  
      this%pphi_start = getpphi(-1 - ds * real(i1 - 1)) ! Left pphi value
      this%pphi_end = getpphi(-1 + ds * real(i2 - 1)) ! Right pphi value
  
      this%npphin = (i2-1) + (i1-1) + 1 ! Number of pphi grid points
  
      ! if grid number > 1
      if (this%npphin .ge. 1) then
          ! Allocate grids - yet to implement
          ...
          do i3 = 1, this%npphin
              ...
              ! Calculate pphi 
              this%pphigrid(i3) = getpphi(pphi_pos)
              ! Store pphi/ei*psi1
              this%s(i3) = pphi_pos
