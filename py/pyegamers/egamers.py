...
class EGAMERS:
    ...
    def read_orbit(self, orbit_name):
        ...
        thetam = np.arange(0., 2*np.pi, 2*np.pi/n)
        xm = np.cos(thetam)
        ym = np.sin(thetam)
    ...
    def plot_fqcmap(self, fqcmapdata=None):
        if fqcmapdata == None:
            ...
            CS = plt.contourf(
                ...
                fqcmapdata.fqcn.reshape(fqcmapdata.ipphin, fqcmapdata.neen),cmap=plt.cm.bone
                )
    ...
    def plot_fqc(self, fqcdata=None):
        ...
        plt.xlabel('r')
        plt.title(r'$\gamma/\omega$ = ' + str(fqcdata.gammaglobal[0]/fqcdata.omgglobal[0]))
