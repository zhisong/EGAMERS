...
def compute_ncfield(self, r=np.linspace(0, 1, 101), tslice=[0], ncfield=None):
    ...
    lambda_CH = CubicHermiteSpline(
        ncfield.r, ncfield.lambda_H1[tslice, :], ncfield.lambda_H2[tslice, :], axis=1
    )
    eta_CH = CubicHermiteSpline(
        ncfield.r, ncfield.eta_H1[tslice, :], ncfield.eta_H2[tslice, :], axis=1
    )
...
def compute_gamma(
    self, tslice_start=0, tslice_end="last", variable="lambda", ncfield=None
):
    ...
    tslice = np.arange(tslice_start, tslice_end + 1)
    t = ncfield.time[tslice]
    eng = ncfield.field_energy[tslice]
    lneng = np.log(eng)

    popt, pcov = curve_fit(lambda x, a, b : a * x + b, t, lneng )

    return popt[0] / 2

def plot_field_energy(
    self, tslice_start=0, tslice_end="last", variable="lambda", log=True, ncfield=None
):
...
def plot_field_radial(
    self, r=np.linspace(0, 1, 101), tslice=[0], variable="lambda", ncfield=None
):
...
def plot_field_time(
    self, r=[0.5], tslice_start=0, tslice_end="last", variable="lambda", ncfield=None
):
    r = np.atleast_1d(r)
    nr = r.size
    if ncfield == None:
